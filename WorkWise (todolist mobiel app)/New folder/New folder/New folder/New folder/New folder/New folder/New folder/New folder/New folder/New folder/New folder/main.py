...
from aimodel import get_efficiency
...
class MainApp(MDApp):
        ...
        try:
            ...
            if incompleted_tasks:
                for task in incompleted_tasks:
                    ...
                    self.list_items.append(add_task)  # Thêm mục vào danh sách công việc
            ...
            Clock.schedule_interval(self.calculate_efficiency, 3600)
    ...
    def edit_task(self, task_text, date, time, task_id):
        ...
        # Tìm kiếm và cập nhật mục danh sách cũ
        list_item = next((item for item in self.list_items if item.pk == task_id), None)
        if list_item:
            list_item.text = '[b]' + task_text + '[/b]'
            list_item.secondary_text = task_date
            db.update_task(task_id, task_text, task_date)
    ...
    def calculate_efficiency(self):
        ...
        if current_hour >= 21:
            ...
            #print(current_date)
            tasks_today = [task for task in self.list_items if datetime.strptime(task.secondary_text, '%A %d %B %Y %H:%M').strftime('%A %d %B %Y') == current_date]
            num_tasks_today = len(tasks_today)
            #print(num_tasks_today)
            #print(len(tasks_today))
            if num_tasks_today==0:
                self.root.ids.efficiency_screen.ids.circular_progress.value = 0
            else:
                efficiency_model = get_efficiency()
                efficiency = efficiency_model.get_eff(num_tasks_today)
                self.root.ids.efficiency_screen.ids.circular_progress.value = efficiency
