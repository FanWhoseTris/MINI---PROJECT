...
class EfficiencyScreen(Screen):
    def update_efficiency(self):
        ...
        all_tasks = app.list_items

        # Calculate the number of completed tasks and total tasks
        done_tasks = sum(1 for task in all_tasks if task.ids.check.active)
        total_tasks = len(all_tasks)

        # Calculate the efficiency using the get_eff method from aimodel.py
        efficiency_calculator = get_efficiency()
        efficiency = efficiency_calculator.get_eff(total_tasks, done_tasks, total_tasks - done_tasks)

        # Update the CircularProgressBar value
        circular_progress_bar = self.ids.circular_progress_bar
        circular_progress_bar.value = efficiency
...
class MainApp(MDApp):
    ...
    def build(self):
        ...
        self.efficiency_screen = EfficiencyScreen()
        self.efficiency_screen.update_efficiency()
        return self.root
    ...
    def on_checkbox_active(self, checkbox, value, list_item):
        # Mark the task as complete when the checkbox is checked
        list_item.mark(checkbox, list_item)
        self.efficiency_screen.update_efficiency()

    def on_checkbox_deactivate(self, checkbox, value, list_item):
        # Mark the task as incomplete when the checkbox is unchecked
        list_item.mark(checkbox, list_item)
        self.efficiency_screen.update_efficiency()
