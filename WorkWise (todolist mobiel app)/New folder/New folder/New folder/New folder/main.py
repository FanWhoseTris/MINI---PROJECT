...
class MainApp(MDApp):
    ...
    def menu_callback(self, text):
        self.menu.dismiss()
        if text == "Chức năng 2":
            self.switch_to_empty_screen()
            self.show_add_task_button = False  # Ẩn nút "plus-thick"
            self.root.ids.add_task_button.opacity = 0  # Cập nhật opacity của nút "plus-thick"

    def switch_to_empty_screen(self):
        self.root.ids.layouts.clear_widgets()
        self.root.ids.layouts.add_widget(EmptyScreen())
